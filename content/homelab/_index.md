...
J'ai tout d'abord étudié la possibilité d'utiliser OPNsense, cependant son usage par le biais des outils devops m'a semblé très compliqué. Notamment la création du template avec packer pour la configuration initile des interfaces.
...
Par la suite j'ai entendu parler de VYos, qui semble supporter une approche IAC, cependant les images par défault n'intègre pas qemu-guest-agent et cloud-init, cependant je suis tombé sur [ce repo Forgejo](https://forge.hutit.fr/vyos/vyos-image-builder) qui présente une manière de construire les images vyos qui supporte qemu-guest-agent et cloud-init. Malheuresement (pour moi) il est basé sur la rooling realse dont on retrouve la phrase suivante dans la [vyos get](https://vyos.net/get/) : "There are no stability guarantees — anything may change, experimental features can be added, and old features can be removed at any time. They are meant for development and testing. " ce qui je l'avoue ne me parait pas très rassurant... La version LTS release n'est accessible que sur demande, gratuite pour entre autre les "non-profit", en revanche la version "Stream" semble être un compromis entre lts et rolling, peut-être que s'il était possible de tweak le repo forgejo présent plus haut pour l'utiliser ce serait une bonne option dans mon cas. 
#### NFtable
...
L'option que j'implémente pour l'instant est l'utilisation de nftable, en envoyant un template [jinja](https://jinja.palletsprojects.com/en/stable/templates/) nftable sur la gateway, ça permet de gérer simplement la partie firewall et routing.
