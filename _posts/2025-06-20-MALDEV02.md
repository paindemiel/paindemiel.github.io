...
Dll hijacking is about placing the .dll so it's loaded, perhaps before the original dll, or if the executable try to load a dll that doesn't exist.

An executable try to load a dll following this order :
1. DLL already loaded in memory
2. Known DLLs 
3. Application's directory -> hacker 
3. `C:\Windows\System32`
4. `C:\Windows\System\`
5. `C:\Windows`
6. Current directory
7. Directories in the system PATH environment variable -> hacker
8. Directories in the user PATH environment variable -> hacker

We can use procmon, search for `path ends with .dll` and `result is NAME NOT FOUND` to search which dll the executable tried to load but did not find, after that we can check folder permissions like `icacls C:\<path>` 
...
Every thread has separate APC queue, The QueueUserAPC function is invoked by application to queue APC to thread, in this call, the contacting thread specifies the address of an APC function .

APC queuing is a request for thread to invoke APC function.

Difference between `VirtualAlloc` and `VirtualAllocEx`, the second one will assign memory within separate process (probably imminent occurence of cross-pross activity).
...
In reallity what this executable does is :
1. it create a legitimate process (notepad.exe)
2. allocate memory
3. write payload
4. APC is enqueued to primary thread in currently suspended state 
5. thread is resume and code is executed 

![maldev5](/assets/images/maldev/2/apc.png)

## NtTestAlert 

Here we will utilize NtTestAlert which is an undocumented funtion which permit us to execute a shellcode within a local procss using Win32 API function calld QueueUserAPC and officially undocumented native api known as NtTestAlert.

NtTestAlert is associated with alerting mechanism of windows OS, this function as potential to initiate execution of any pending APCs associated with thread.
...
API hooking is a method that's used to manipulate and alter functionality and sequence of API calls. This technique is frequently used by different AV to identify whether piece of code is malicious or not.

So we have a dll which export functions (Cat,Mouse,Frog and Bird) which accept single parameter denoted as `message`.

All message box will be displayed one after the other.

Here is the cat.c which can be compiled with :
...
and the dll pet.dll which can be compiled with :
